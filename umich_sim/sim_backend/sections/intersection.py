...
# Local Imports
from umich_sim.sim_backend.helpers import (to_numpy_vector,
                                           project_forward,
                                           angle_difference)
...
class Intersection(Section):
    ...
    def stop_at_light(self, current_vehicle: Vehicle, braking_distance: float) -> Tuple[bool, carla.Location]:
        ...
        if current_separation < braking_distance:
            ...
            if self.traffic_lights[light_index].get_state() in [carla.TrafficLightState.Red,
    ...
    def get_thru_waypoints(self, carla_map: carla.Map, current_vehicle: Vehicle,
        ...
        possible_waypoint_pairs: List[Tuple[carla.Waypoint, carla.Waypoint]] = self.junction.get_waypoints(
        ...
        possible_waypoint_pairs = list(
            filter(lambda x: np.linalg.norm(
                to_numpy_vector(x[0].transform.location) - to_numpy_vector(starting_waypoint.transform.location)) < 1.0,
        ...
        closest_yaw = np.argmin(
            [angle_difference(x[1].transform.rotation.yaw, target_yaw) for x in possible_waypoint_pairs])
        ...
        if direction in ("left", "right"):
            ...
            waypoints.insert(1, carla_map.get_waypoint(carla.Location(
                x=starting_waypoint.transform.location.x + (
                        (thru_waypoint.transform.location.x - starting_waypoint.transform.location.x) / 2),
                y=starting_waypoint.transform.location.y + (
                        (thru_waypoint.transform.location.y - starting_waypoint.transform.location.y) / 2),
                z=0.0
            )))
